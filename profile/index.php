...
if(empty($_SESSION['email'])){
  ...
  foreach($taikhoan1 as $key=>$value) { $ten=$value['HOTEN'].' '.'★'; $tach_ten = explode(" ",
$ten); if ($tach_ten[1]=='★'){ $account=$tach_ten[0]; } else if ($tach_ten[2]=='★'){
$account=$tach_ten[0].' '.$tach_ten[1]; } else { $account=$tach_ten[1].' '.$tach_ten[2]; } }
$dsyeuthich=mysqli_query($conn,"SELECT * FROM yeuthich WHERE email_khachhang='$email'");
$sql="SELECT * FROM khachhang WHERE email='$email'"; $query=mysqli_query($conn,$sql); $taikhoan=
mysqli_fetch_assoc($query); if (isset($_POST['account'])){ require_once("index.php"); } if
(isset($_POST['luu'])){ $HOTEN=$_POST['HOTEN']; $CMND=$_POST['CMND']; $day=$_POST['day'];
$month=$_POST['month']; $year=$_POST['year']; $SODIENTHOAI=$_POST['SODIENTHOAI'];
$MOTA=$_POST['MOTA']; $email=$_SESSION['email']; $sql="UPDATE khachhang SET
HOTEN='$HOTEN',CMND='$CMND',NGAY='$day',THANG='$month',NAM='$year',SODIENTHOAI='$SODIENTHOAI',MOTA='$MOTA'
WHERE email='$email'"; $query=mysqli_query($conn,$sql); if ($query==false){ echo "lỗi"; }else {
header("location:index.php"); } } if (isset($_POST['thaydoi'])){ $error=''; $old=md5($_POST['old']);
$new=md5($_POST['new']); $renew=md5($_POST['renew']); if(empty($old)){ $error='Bạn chưa nhập mật
khẩu cũ'; } else if(empty($new)){ $error='Bạn chưa nhập mật khẩu mới'; }else if($new!=$renew){
$error='Mật khẩu nhập lại không đúng'; } else if ($old!=$taikhoan['password']){ $error='Mật khẩu cũ
không đúng'; }else if (!empty($error)){ } else if (empty($error)){ $sql="UPDATE khachhang SET
password= '$new' WHERE email='$email'"; $query=mysqli_query($conn,$sql); if ($query){
header("location:index.php"); } } }else if (isset($_POST['huy'])){ header("location:index.php"); }
  ...
  <body>
    <form action="" method="post">
      ...
      <div class="wrapper">
        ...
        <section class="user">
          <div class="container">
            <h1 class="mb-5 global-heading global-heading--big">Account Settings</h1>
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6">
                      <div class="form-group">
                        ...
                        <input
                          ...
                          class="form-control"
                          name="SODIENTHOAI"
                          value="<?php echo $taikhoan['SODIENTHOAI']?>"
                        />
                      </div>
                    </div>
                    ...
                    <div class="col-md-12">
                      <div class="form-group">
                        ...
                        <textarea class="form-control" name="MOTA" rows="4">
<?php echo $taikhoan['MOTA']?></textarea
                        >
                      </div>
                    </div>
                  </div>
...
>>>>>>> 4a7c6aae1e1259e5231e11cf5a75c6ac74e4ab66
                      </div>
                    </div>
                    <div class="accordion-item">
